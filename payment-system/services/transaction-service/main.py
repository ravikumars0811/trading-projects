...
logger = setup_logging("transaction-service")
...
REQUEST_LATENCY = Histogram(
    'transaction_service_request_duration_seconds',
    'Request latency',
    ['method', 'endpoint']
)

app = FastAPI(
    title="Payment System - Transaction Service",
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={"success": False, "error": "Internal server error", "details": None}
    )
...
@app.get("/")
async def root():
    return {"service": "transaction-service", "version": "1.0.0", "status": "running"}
